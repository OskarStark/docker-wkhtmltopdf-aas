    ...
    if options:
        for option, value in options.items():
            ...
            if option == 'header-html':
                ...
                header_file.write(value.decode('base64'))
                header_file.flush()
                args.append('"%s"' % header_file.name)
            elif option == 'footer-html':
                footer_file = tempfile.NamedTemporaryFile(suffix='.html')
                footer_file.write(value.decode('base64'))
                footer_file.flush()
                args.append('"%s"' % footer_file.name)
            elif value:
                args.append('"%s"' % value)
